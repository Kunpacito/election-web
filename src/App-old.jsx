...
const STORAGE_KEY = "election-data";
...
export default function App() {
  ...
  return <VoterPage election={election} onSave={saveElection} onRefresh={refresh} onBack={() => setRole(null)} />;
}
      ...
      <div style={S.pageWrap}>
        ...
        <div style={{ textAlign: "center", marginTop: 6 }}>
          <span style={{ fontSize: 11, color: saved ? "#4ade80" : "#475569" }}>
            {saving ? "กำลังบันทึก..." : saved ? "✓ บันทึกแล้ว" : "auto-save"}
          </span>
        </div>
      </div>
    </div>
...
// ─── VOTER PAGE ───────────────────────────────────────────────
function VoterPage({ election, onSave, onRefresh, onBack }) {
  ...
  const vote = async (id) => {
    ...
    await onSave({ ...election, votes: updated });
    setHasVoted(true);
    setVotedFor(id);
    onRefresh();
  };
      ...
      <div style={S.root}>
        ...
        <div style={S.pageWrap}>
          ...
          <div style={{ textAlign: "center", marginTop: 80, color: "#64748b" }}>
            ...
            <p style={{ fontSize: 13, color: "#475569" }}>หน้านี้จะ refresh อัตโนมัติ</p>
          </div>
        </div>
      </div>
    );
      ...
      <div style={S.root}>
        ...
        <div style={S.pageWrap}>
          ...
          <div style={{ textAlign: "center", marginTop: 60 }}>
            ...
            <p style={{ fontSize: 12, color: "#475569", marginTop: 4 }}>หน้านี้จะ refresh อัตโนมัติ</p>
          </div>
      ...
      <div style={S.pageWrap}>
        ...
        {showResults && (
          <>
            ...
            <div style={S.card}>
              <label style={S.label}>คะแนนทั้งหมด ({totalVotes})</label>
              {sortedCandidates.map((c) => {
                const pct = maxVotes > 0 ? ((votes[c.id] || 0) / maxVotes) * 100 : 0;
                const isW = c.id === winner?.id;
                return (
            ...
            <p style={{ textAlign: "center", fontSize: 11, color: "#475569", marginTop: 6 }}>ผลนี้ update อัตโนมัติครับ</p>
          </>
        )}
